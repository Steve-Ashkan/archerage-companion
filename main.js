...
// Dev-mode only — gives dev role when running from source (npm start).
// Never fires in packaged builds, so no installed user is affected.
function applyDevModeRole(profile) {
  if (app.isPackaged) return profile;
  return { ...profile, role: 'dev', source: 'dev-mode' };
}
...
async function getCallerRole() {
  try {
    ...
    const prof = await getProfile(session);
...
function sanitizeScanItemName(itemName) {
  return String(itemName || '').trim().replace(/[\r\n,]/g, '').slice(0, 120);
}

function isValidScanItemName(itemName) {
  if (!itemName || itemName.length > 120) return false;
  if (/[;]/.test(itemName)) return false;
  if (/(--|\/\*|\*\/)/.test(itemName)) return false;
  if (/\b(drop|delete|insert|update|alter|create|truncate)\b/i.test(itemName)) return false;
  return true;
}
...
ipcMain.handle('add-to-scan-list', async (event, { itemName }) => {
  try {
    ...
    const safeName = sanitizeScanItemName(itemName);
...
function profileFromSession(session, reason = null) {
  const payload = session?.sessionToken ? decodeJWTPayload(session.sessionToken) : null;
  // Role MUST come from a verified server response — never trust the unverified JWT payload.
  // Only display fields (name, avatar) are safe to read from the local token.
  return applyDevModeRole({
    role:        'free',
    proExpires:  null,
    ign:         null,
    discordName: payload?.name   || session?.discordName || null,
    avatar:      payload?.avatar || session?.avatar      || null,
    source:      'token-display-only',
    error:       reason,
  });
}

async function getProfile(session = loadSession()) {
  try {
    const data = await callAppApi('get-profile');
    if (data.ok && data.profile) return applyDevModeRole({
    ...
    console.warn('[auth] get-profile returned no profile:', data?.error || data);
    return profileFromSession(session, data?.error || 'No profile returned');
  } catch(e) {
    console.warn('[auth] get-profile failed:', e.message);
    return profileFromSession(session, e.message);
  }
}
...
ipcMain.handle('get-auth-status', async () => {
  try {
    ...
    return {
      ...
      authSource: prof.source || 'api',
      authError: prof.error || null,
    };
  } catch(e) {
...
ipcMain.handle('open-discord-auth', async () => {
  try {
    // Generate a fresh state nonce per login attempt (one-time use, stored in Set)
    ...
    // Open Discord OAuth in the system browser. Discord's protocol handler does not
    // support OAuth authorization URLs — discord:// only handles internal navigation.
    // The browser is the only reliable path: user clicks Authorize (one click if
    // already logged into Discord), super-endpoint mints the JWT, redirects to
    // archerage://auth/callback, and the second-instance handler delivers it here.
    shell.openExternal(`https://discord.com/oauth2/authorize?${params}`);
    return { ok: true };
  } catch(e) {
...
ipcMain.handle('handle-oauth-callback', async (event, callbackUrl) => {
  try {
    ...
    const prof  = await getProfile({ discordId, discordName: name, avatar, sessionToken: token });
    ...
    return {
      ...
      auth: {
        ...
        isPro,
        role: prof.role,
        user: { id: discordId, name: prof.discordName || name, avatar: prof.avatar || avatar },
        proExpires: prof.proExpires || null,
        authSource: prof.source || 'api',
        authError: prof.error || null,
      },
...
// Optional supporter recognition log
ipcMain.handle('donations-get-public', async (event, { month } = {}) => {
  try {
    return await callAppApi('donations-get-public', { month });
  } catch(e) { return { ok: false, donations: [], error: e.message }; }
});

ipcMain.handle('donations-admin-add', async (event, donation) => {
  try {
    return await callAppApi('donations-admin-add', donation || {});
  } catch(e) { return { ok: false, error: e.message }; }
});

ipcMain.handle('donations-admin-delete', async (event, { id }) => {
  try {
    return await callAppApi('donations-admin-delete', { id });
  } catch(e) { return { ok: false, error: e.message }; }
});
...
app.whenReady().then(() => {
  ...
  if (app.isPackaged) {
    ...
    autoUpdater.on('update-available', (info) => {
      ...
      const wins = BrowserWindow.getAllWindows();
      if (wins[0]) wins[0].webContents.send('update-available', {
    ...
    autoUpdater.on('download-progress', (p) => {
      ...
      const wins = BrowserWindow.getAllWindows();
      if (wins[0]) wins[0].webContents.send('update-download-progress', { percent });
    });
