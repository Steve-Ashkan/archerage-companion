...
function canReviewFlaggedPrices(userRole: string): boolean {
  return ['curator', 'admin', 'dev'].includes(userRole);
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'donations-get-public') {
      const month = monthKey(params.month);
      const { data, error } = await db
        .from('donation_log')
        .select('id, display_name, ign, support_type, tier, note, donated_on, donation_month')
        .eq('is_public', true)
        .eq('donation_month', month)
        .order('donated_on', { ascending: false })
        .limit(50);
      if (error) return json({ ok: false, donations: [], error: error.message });
      return json({ ok: true, donations: data || [], month });
    }
    ...
    if (action === 'donations-admin-add') {
      if (!hasRole(callerRole, 'admin')) return json({ ok: false, error: 'Requires admin role' }, 403);
      const displayName = str(params.displayName, 120);
      const ign = str(params.ign, 80);
      const supportType = str(params.type, 40) || 'Other';
      const amountPrivate = str(params.amount, 120);
      const tier = str(params.tier, 80) || 'Supporter';
      const note = str(params.note, 240);
      const donatedOn = str(params.date, 20) || new Date().toISOString().slice(0, 10);
      if (!displayName && !ign) return json({ ok: false, error: 'Missing supporter name or IGN' });
      if (!/^\d{4}-\d{2}-\d{2}$/.test(donatedOn)) return json({ ok: false, error: 'Invalid date' });
      ...
      const { data, error } = await db
        ...
        .insert({
          ...
          amount_private: amountPrivate,
          tier,
          note,
          is_public: params.public !== false,
          donated_on: donatedOn,
          donation_month: donatedOn.slice(0, 7),
          created_by: discord_id,
        })
        .select('id, display_name, ign, support_type, amount_private, tier, note, is_public, donated_on, donation_month, created_at')
        .single();
      if (error) return json({ ok: false, error: error.message });
      return json({ ok: true, donation: data });
    }

    if (action === 'donations-admin-delete') {
      if (!hasRole(callerRole, 'admin')) return json({ ok: false, error: 'Requires admin role' }, 403);
      const id = numId(params.id);
      if (!id) return json({ ok: false, error: 'Invalid id' });
      const { error } = await db.from('donation_log').delete().eq('id', id);
      if (error) return json({ ok: false, error: error.message });
      return json({ ok: true });
    }

    if (action === 'admin-get-flagged-prices') {
      if (!canReviewFlaggedPrices(callerRole)) return json({ ok: false, error: 'Requires curator, admin, or dev role' }, 403);
