...
function renderAHScanner() {
  return section('ah-scanner', '💰', 'AH Scanner', `
    ...
    <div style="background:#1a1000;border:1px solid #fcd34d;border-radius:10px;padding:14px 16px;margin-top:10px;">
      <div style="font-size:13px;font-weight:700;color:#fcd34d;margin-bottom:6px;">⭐ Free & Donations — AH Scanner</div>
      <div style="font-size:13px;color:#94a3b8;line-height:1.7;">
        <strong style="color:#86efac;">All users:</strong> The AH Scanner addon is completely free to install and run in-game.
        However, the <strong>Import AH Prices</strong> button in the app is free to use. Click <strong>Import AH Prices</strong> and all prices update from your latest scan automatically.
      </div>
    </div>
  `);
}
...
function renderInvScanner() {
  return section('inv-scanner', '🎒', 'Inventory Scanner', `
    ...
    <div style="background:#1a1000;border:1px solid #fcd34d;border-radius:10px;padding:14px 16px;margin-top:10px;">
      <div style="font-size:13px;font-weight:700;color:#fcd34d;margin-bottom:6px;">⭐ Free & Donations — Inventory Scanner</div>
      <div style="font-size:13px;color:#94a3b8;line-height:1.7;">
        <strong style="color:#86efac;">All users:</strong> You can run the scanner in-game and it works fully — bag, bank, guild bank, and coffer all scan correctly.
        However, the <strong>Import Inventory</strong> button in the app is free to use. Click <strong>Import Inventory</strong> after scanning and all your quantities update instantly.
      </div>
    </div>
  `);
}
...
function renderPricesStorage() {
  return section('prices-storage', '📦', 'Prices & Storage', `
    ...
    <div style="background:#1a1000;border:1px solid #fcd34d;border-radius:10px;padding:14px 16px;margin-bottom:16px;">
      <div style="font-size:13px;font-weight:700;color:#fcd34d;margin-bottom:6px;">⭐ Free & Donations — Prices &amp; Storage</div>
      <div style="font-size:13px;color:#94a3b8;line-height:1.7;">
        <strong style="color:#86efac;">All users</strong> can view all items and enter prices and quantities manually by clicking any field. You can run the AH Scanner and Inventory Scanner in-game and import their results directly. Everyone gets one-click imports from both scanners, auto-updated community prices, and the ability to add custom items.
      </div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
      <div style="padding:10px 14px;background:#1a2028;border:1px solid #2a3040;border-radius:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
          ...
          <span style="font-size:10px;font-weight:700;padding:1px 8px;border-radius:10px;background:#3a3018;color:#ffd166;border:1px solid #5a4a28;">FREE</span>
        </div>
      ...
      <div style="padding:10px 14px;background:#1a2028;border:1px solid #2a3040;border-radius:8px;">
        ...
        <div style="font-size:13px;color:#8d99ab;">Prices submitted by other players and verified by Ashkan can auto-fill for everyone.</div>
      </div>
      <div style="padding:10px 14px;background:#1a2028;border:1px solid #2a3040;border-radius:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
...
function renderArcPoints() {
  return section('arc-points', '✦', 'ARC Points', `
    <p style="font-size:13px;color:#94a3b8;margin:0 0 16px;line-height:1.6;">
      ...
      spend them on cosmetic app unlocks and community recognition.
    </p>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:16px;">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:6px;">
          ${[
            ['Profile titles', '250+ pts'],
            ['Badges & themes', '400+ pts'],
            ['Profile banners', '1,000+ pts'],
          ].map(([label, cost]) => `
            <div style="display:flex;justify-content:space-between;align-items:center;
              padding:8px 12px;background:#1a2028;border:1px solid #2a3040;border-radius:8px;">
    ...
    ${tip('ARC Points are a thank-you for helping the community with guides, recipes, and price data.')}
  `);
}
...
function renderSupport() {
  return section('support', '⭐', 'Free & Donations', `
    <p style="font-size:13px;color:#94a3b8;margin:0 0 16px;line-height:1.6;">
      ArcheRage Companion is free for the community. There is no subscription, no required payment, and no feature paywall.
    </p>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:16px;">
      ...
      <div>
        <div style="font-size:10px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;
          color:#a07a10;margin-bottom:8px;">Included</div>
        ${[
          'Net Worth tracker',
          'Bag & Vault Scanner',
    ...
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;
      color:#566174;margin-bottom:10px;">Optional donations</div>

    <div style="display:flex;flex-direction:column;gap:8px;">
      <div style="padding:10px 14px;background:#1a1a0a;border:1px solid #3a3018;border-radius:8px;">
        <div style="font-size:13px;font-weight:600;color:#ffd166;margin-bottom:3px;">Voluntary support</div>
        <div style="font-size:13px;color:#8d99ab;">Donations are optional and never required to use app features.</div>
      </div>
      <div style="padding:10px 14px;background:#0a1a1a;border:1px solid #1a3a3a;border-radius:8px;">
        <div style="font-size:13px;font-weight:600;color:#86efac;margin-bottom:3px;">In-game support</div>
        <div style="font-size:13px;color:#8d99ab;">If someone wants to support development in-game, they can mail Ashkan voluntarily.</div>
      </div>
      <div style="padding:10px 14px;background:#0a0a1a;border:1px solid #1a1a3a;border-radius:8px;">
        <div style="font-size:13px;font-weight:600;color:#93c5fd;margin-bottom:3px;">ARC Points</div>
        <div style="font-size:13px;color:#8d99ab;">Earn points by submitting guides and recipes. Redeem for cosmetic app unlocks.</div>
      </div>
    </div>
  `);
}
