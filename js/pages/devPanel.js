...
function isValidScanItemName(itemName) {
  const name = String(itemName || '').trim();
  if (!name || name.length > 120) return false;
  if (/[;\r\n,]/.test(name)) return false;
  if (/(--|\/\*|\*\/)/.test(name)) return false;
  if (/\b(drop|delete|insert|update|alter|create|truncate)\b/i.test(name)) return false;
  return true;
}

function canReviewFlaggedPrices(role) {
  return ['admin', 'dev', 'curator'].includes(role);
}
...
function canManagePendingPriceItems(role) {
  return role === 'dev';
}

function canReviewSubmissions(role) {
  return ROLE_LEVEL[role] >= ROLE_LEVEL.curator;
}

function canManageRedemptions(role) {
  return ROLE_LEVEL[role] >= ROLE_LEVEL.admin;
}

function canManageDonations(role) {
  return ROLE_LEVEL[role] >= ROLE_LEVEL.admin;
}

function canSendDevMail(role) {
  return ROLE_LEVEL[role] >= ROLE_LEVEL.staff;
}

const DONATION_LOG_KEY = 'donationLog';
...
const DP_PAGE_SIZE = 25;
const _dpPageState = {};
const _dpPageData = {};

function paginateItems(key, items) {
  const list = Array.isArray(items) ? items : [];
  const totalPages = Math.max(1, Math.ceil(list.length / DP_PAGE_SIZE));
  const page = Math.min(Math.max(_dpPageState[key] || 1, 1), totalPages);
  _dpPageState[key] = page;
  const start = (page - 1) * DP_PAGE_SIZE;
  return {
    items: list.slice(start, start + DP_PAGE_SIZE),
    page,
    totalPages,
    start,
    total: list.length,
...
function renderPager(key, pageInfo) {
  const first = pageInfo.total ? pageInfo.start + 1 : 0;
  const last = Math.min(pageInfo.start + DP_PAGE_SIZE, pageInfo.total);
  const disabledPrev = pageInfo.page <= 1 ? 'disabled' : '';
  const disabledNext = pageInfo.page >= pageInfo.totalPages ? 'disabled' : '';
  return `
    <div class="dp-pager">
      <span>Showing ${first}-${last} of ${pageInfo.total}</span>
      <div class="dp-pager-buttons">
        <button class="dp-btn dp-page-btn" style="padding:4px 10px;font-size:12px;"
          data-page-key="${escapeHtml(key)}" data-page="${pageInfo.page - 1}" ${disabledPrev}>Prev</button>
        <span>Page ${pageInfo.page} / ${pageInfo.totalPages}</span>
        <button class="dp-btn dp-page-btn" style="padding:4px 10px;font-size:12px;"
          data-page-key="${escapeHtml(key)}" data-page="${pageInfo.page + 1}" ${disabledNext}>Next</button>
      </div>
    </div>
  `;
}
...
function renderPaginatedSection(key) {
  ...
  const wrap = document.getElementById(wrapByKey[key]);
  const render = renderByKey[key];
  if (!wrap || !render) return;
  wrap.innerHTML = render(_dpPageData[key] || []);
}
...
function saveDonationLog(entries) {
  localStorage.setItem(DONATION_LOG_KEY, JSON.stringify(entries));
}
...
function donationMonth(dateValue) {
  return String(dateValue || currentDateInputValue()).slice(0, 7);
}

function renderDonationLog(entries = getDonationLog()) {
  ...
  if (!monthEntries.length) {
    return '<p style="color:#566174;font-size:13px;margin:12px 0 0;">No donations logged for this month yet.</p>';
  }
  ...
  const rows = monthEntries.map(entry => `
    <tr>
      <td>
        <strong style="color:#eef2f7;">${escapeHtml(entry.displayName || entry.ign || 'Anonymous')}</strong>
        ${entry.ign ? `<div style="font-size:11px;color:#566174;">IGN: ${escapeHtml(entry.ign)}</div>` : ''}
      </td>
      <td>${escapeHtml(entry.type || 'Other')}</td>
      <td>${escapeHtml(entry.amount || 'Private')}</td>
      <td>${escapeHtml(entry.tier || 'Supporter')}</td>
      <td>${entry.public ? '<span style="color:#86efac;">Public</span>' : '<span style="color:#566174;">Private</span>'}</td>
      <td>${escapeHtml(entry.date || '')}</td>
      <td>
        <button class="dp-btn danger" style="padding:4px 10px;font-size:11px;"
          onclick="window.deleteDonationEntry('${jsEscape(entry.id)}')">Delete</button>
      </td>
    </tr>
  `).join('');
  ...
  return `
    <div style="margin-top:16px;">
      <div style="font-size:12px;color:#566174;margin-bottom:8px;">
        ${escapeHtml(currentMonth)} totals: ${summary}
      </div>
...
export function renderPage() {
  ...
  return `
    <style>
      ...
      .dp-user-table th,
      .dp-table th { background: #1a2535; color: #566174; font-size: 11px;
        text-transform: uppercase; letter-spacing: 0.08em; padding: 8px 12px; text-align: left; }
      .dp-user-table td,
      .dp-table td { padding: 10px 12px; border-bottom: 1px solid #1e2d3d; color: #cbd5e1;
        vertical-align: middle; }
      .dp-user-table tbody tr,
      .dp-table tbody tr { transition: background 0.12s ease, box-shadow 0.12s ease; }
      .dp-user-table tbody tr:hover,
      .dp-table tbody tr:hover {
        background: #20334a;
        box-shadow: inset 3px 0 0 #38bdf8;
      }
      .dp-user-table tbody tr:hover td,
      .dp-table tbody tr:hover td { color: #eef2f7; }
      ...
      .dp-pager { display:flex;justify-content:space-between;align-items:center;gap:12px;
        margin:12px 0 0;color:#566174;font-size:12px; }
      .dp-pager-buttons { display:flex;align-items:center;gap:8px; }
    </style>
    ...
    <div class="dp-grid">
      ...
      <!-- ── Role Switcher ────────────────────────────────────────────────── -->
      <div class="card" style="${role === 'dev' ? '' : 'display:none;'}">
      ...
      <!-- ── User Management ─────────────────────────────────────────────── -->
      <div class="card dp-grid-full" style="${canManageUsers(role) ? '' : 'display:none;'}">
      ...
      <!-- ── Pending Price Items ───────────────────────────────────────────── -->
      <div class="card dp-grid-full" style="${canManagePendingPriceItems(role) ? '' : 'display:none;'}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      ...
      <!-- ── Flagged Prices ────────────────────────────────────────────────── -->
      <div class="card dp-grid-full" style="${canReviewFlaggedPrices(role) ? '' : 'display:none;'}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      ...
      <!-- ── Redemption Queue ────────────────────────────────────────────── -->
      <div class="card dp-grid-full" style="${canManageRedemptions(role) ? '' : 'display:none;'}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      ...
      <!-- ── Send Mail ───────────────────────────────────────────────────── -->
      <!-- Donation Log -->
      <div class="card dp-grid-full" style="${canManageDonations(role) ? '' : 'display:none;'}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          <h3 style="margin:0;">Donation Log</h3>
          <button class="dp-btn" onclick="window.refreshDonationLog()">Refresh</button>
        </div>
        <p style="color:#566174;font-size:13px;margin:0 0 14px;">
          Manual recognition log for optional support. Amounts stay here; public entries can appear on the monthly Home supporter board.
        </p>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px;margin-bottom:10px;">
          <div>
            <div class="dp-label">Display Name</div>
            <input id="dp-donor-name" type="text" placeholder="Discord or display name"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          </div>
          <div>
            <div class="dp-label">IGN</div>
            <input id="dp-donor-ign" type="text" placeholder="Optional"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          </div>
          <div>
            <div class="dp-label">Type</div>
            <select id="dp-donation-type"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          ...
          <div>
            <div class="dp-label">Amount / Item</div>
            <input id="dp-donation-amount" type="text" placeholder="e.g. 500 credits"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          </div>
          <div>
            <div class="dp-label">Date</div>
            <input id="dp-donation-date" type="date" value="${currentDateInputValue()}"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          </div>
          <div>
            <div class="dp-label">Public Tier</div>
            <select id="dp-donation-tier"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
              ${DONATION_TIERS.map(tier => `<option>${escapeHtml(tier)}</option>`).join('')}
            </select>
          </div>
        </div>

        <div style="display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:10px;align-items:end;">
          <div>
            <div class="dp-label">Public Note</div>
            <input id="dp-donation-note" type="text" placeholder="Optional thank-you note shown on Home"
              style="width:100%;box-sizing:border-box;padding:8px 12px;background:#0f1923;border:1px solid #2a3a52;color:#eef2f7;border-radius:8px;font-size:13px;">
          </div>
          <label style="display:flex;align-items:center;gap:8px;color:#94a3b8;font-size:13px;padding-bottom:8px;">
            <input id="dp-donation-public" type="checkbox" checked>
            Show on Home
          </label>
        </div>

        <div style="margin-top:12px;">
          <button class="dp-btn success" onclick="window.addDonationEntry()">Add Donation</button>
          <span id="dp-donation-status" style="font-size:12px;margin-left:12px;"></span>
        </div>
        <div id="dp-donation-wrap">${renderDonationLog(getDonationLog())}</div>
      </div>

      <div class="card dp-grid-full" style="${canSendDevMail(role) ? '' : 'display:none;'}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
...
export function initDevPanel() {
  const role = getRole();
  if (canManageUsers(role)) window.loadAdminUsers();
  ...
  document.querySelector('.dp-grid')?.addEventListener('click', (e) => {
    const pageBtn = e.target.closest('.dp-page-btn');
    if (pageBtn) {
      _dpPageState[pageBtn.dataset.pageKey] = Number(pageBtn.dataset.page) || 1;
      renderPaginatedSection(pageBtn.dataset.pageKey);
      return;
    }
...
window.refreshDonationLog = function() {
  const wrap = document.getElementById('dp-donation-wrap');
  if (wrap) wrap.innerHTML = renderDonationLog(getDonationLog());
};

window.addDonationEntry = async function() {
  const nameEl   = document.getElementById('dp-donor-name');
  const ignEl    = document.getElementById('dp-donor-ign');
  const typeEl   = document.getElementById('dp-donation-type');
  const amountEl = document.getElementById('dp-donation-amount');
  const dateEl   = document.getElementById('dp-donation-date');
  const tierEl   = document.getElementById('dp-donation-tier');
  const publicEl = document.getElementById('dp-donation-public');
  const noteEl   = document.getElementById('dp-donation-note');
  const status   = document.getElementById('dp-donation-status');

  const displayName = String(nameEl?.value || '').trim();
  const ign = String(ignEl?.value || '').trim();
  const date = String(dateEl?.value || currentDateInputValue()).trim();

  if (!displayName && !ign) {
    if (status) { status.style.color = '#f87171'; status.textContent = 'Add a display name or IGN first.'; }
    return;
  }
  ...
  const entry = {
    ...
    displayName,
    ign,
    type: String(typeEl?.value || 'Other').trim(),
    amount: String(amountEl?.value || '').trim(),
    date,
    month: donationMonth(date),
    tier: String(tierEl?.value || 'Supporter').trim(),
    public: Boolean(publicEl?.checked),
  ...
  let pushed = false;
  let pushError = '';
  if (window.electronAPI?.donationsAdminAdd) {
    const result = await window.electronAPI.donationsAdminAdd(entry);
    if (result?.ok) {
      pushed = true;
      if (result.donation?.id) entry.id = String(result.donation.id);
    } else {
  ...
  saveDonationLog(entries);

  if (nameEl) nameEl.value = '';
  if (ignEl) ignEl.value = '';
  if (amountEl) amountEl.value = '';
  if (noteEl) noteEl.value = '';
  if (dateEl) dateEl.value = currentDateInputValue();
  if (publicEl) publicEl.checked = true;
  window.refreshDonationLog();

  if (status) {
    status.style.color = pushed ? '#86efac' : '#fcd34d';
    status.textContent = pushed ? 'Donation pushed and logged.' : `Saved locally only: ${pushError || 'cloud push unavailable'}`;
    setTimeout(() => { status.textContent = ''; }, 2500);
  }
};

window.deleteDonationEntry = async function(id) {
  if (!confirm('Delete this donation log entry?')) return;
  if (/^\d+$/.test(String(id)) && window.electronAPI?.donationsAdminDelete) {
    await window.electronAPI.donationsAdminDelete(id);
  }
  const entries = getDonationLog().filter(entry => entry.id !== id);
  saveDonationLog(entries);
  window.refreshDonationLog();
};
...
function renderFlaggedPricesTable(items) {
  ...
  const pageInfo = paginateItems('flagged', items);
...
window.loadPendingPriceItems = async function() {
  ...
  _dpPageData.pending = result.items || [];
  _dpPageState.pending = 1;
  wrap.innerHTML = renderPendingPriceItems(_dpPageData.pending);
};

window.addAllPendingToScanList = async function() {
  const wrap = document.getElementById('dp-pending-wrap');
  const btn  = document.getElementById('dp-pending-add-all-btn');
  const items = _dpPageData.pending || [];
  ...
  status.textContent = `Added ${added} item${added === 1 ? '' : 's'} to scan_items.csv${skipped ? `, skipped ${skipped} invalid` : ''}`;
...
window.rejectPendingItem = async function(btn, itemName) {
  ...
  if (result?.ok) {
    _dpPageData.pending = (_dpPageData.pending || []).filter(item => item.item_name !== itemName);
    renderPaginatedSection('pending');
  } else {
...
window.addPendingItemToScanList = async function(btn, itemName) {
  if (!isValidScanItemName(itemName)) {
  ...
  if (result?.ok) {
    ...
    btn.textContent = result?.reason === 'exists'
      ? 'Already in list'
...
function renderPendingPriceItems(items) {
  ...
  const pageInfo = paginateItems('pending', items);
  const rows = pageInfo.items.map(item => {
    const itemName = String(item.item_name || '');
    const valid = isValidScanItemName(itemName);
    return `
    <tr data-pending-item="${escapeHtml(itemName)}" style="border-bottom:1px solid #1e2a3a;">
      <td style="padding:10px 12px;color:#eef2f7;">
        ${escapeHtml(itemName)}
        ${valid ? '' : `
          <span style="display:inline-block;margin-left:8px;padding:2px 8px;border-radius:20px;
            background:#2a1a1a;color:#f87171;border:1px solid #5a2a2a;font-size:11px;font-weight:700;">
            Invalid
          </span>
        `}
      </td>
      ...
      <td style="padding:10px 12px;">
        <div style="display:flex;gap:6px;">
          <button class="dp-btn success dp-pending-add-btn" style="padding:4px 12px;font-size:12px;"
            data-item-name="${escapeHtml(itemName)}" ${valid ? '' : 'disabled'}>
            ${valid ? 'Add to Scan List' : 'Blocked'}
          </button>
...
window.loadFlaggedPrices = async function() {
  ...
  _dpPageData.flagged = result.items || [];
  _dpPageState.flagged = 1;
  wrap.innerHTML = renderFlaggedPricesTable(_dpPageData.flagged);
};
...
function renderRecipeSubmissions(items) {
  ...
  const pageInfo = paginateItems('recipe', items);
  ...
  return cards + renderPager('recipe', pageInfo);
}
...
window.loadRecipeSubmissions = async function() {
  ...
  _dpPageData.recipe = result.submissions || [];
  _dpPageState.recipe = 1;
  wrap.innerHTML = renderRecipeSubmissions(_dpPageData.recipe);
};
...
function renderRedemptionQueue(items) {
  ...
  const pageInfo = paginateItems('redeem', items);

  const rows = pageInfo.items.map(r => {
...
window.loadRedemptionQueue = async function() {
  ...
  _dpPageData.redeem = result.redemptions || [];
  _dpPageState.redeem = 1;
  wrap.innerHTML = renderRedemptionQueue(_dpPageData.redeem);
};
