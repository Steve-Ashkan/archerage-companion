...
function getEventOccurrence(event, now) {
  ...
  // Bug fix: return the soonest future occurrence found in the 14-day window.
  // Previously fell through to the fallback below, which made every non-active
  // event show "Next: tomorrow midnight UTC" (= 19:00/20:00 ET).
  if (next) return next;
